...
GITIGNORE = """\
# Generated by hills. This excludes .autolab/ from the surrounding project's git,
...
def new(name: str, *, template: str = DEFAULT_TEMPLATE, where: Path | None = None) -> Hill:
    base = paths.project_hills(where or Path.cwd())
    root = base / name
    if root.exists():
        raise HillsError(f"{root} already exists")

    base.mkdir(parents=True, exist_ok=True)
    # One level above the hills, so the whole .autolab/ tree stays out of the
    # surrounding project's git.
