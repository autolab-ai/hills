...
**Environment (spec v3).** By default the evaluator runs in the hill's uv environment, resolved from its `pyproject.toml`. A hill may instead run the evaluator inside a container image by declaring, at `spec_version: 3`:

```yaml
environment:
  image: ghcr.io/you/your-image@sha256:...
```

When set, `hills eval` (and `hills check`'s tests) run inside that image using whatever container runtime is on PATH: docker or podman locally, apptainer or singularity on HPC (both daemonless siblings with the same CLI), which run the OCI image via the `docker://` transport, converting it on the fly. The runtime is auto-detected (docker → podman → apptainer → singularity), but the caller sets policy, never the tool: `--runtime` on the CLI or `HILLS_RUNTIME` in the environment (the platform sets it per node) forces one, and a forced-but-unusable runtime is a clear error rather than a silent substitution. A special runtime, **`--runtime host`** (or `HILLS_RUNTIME=host`), runs the evaluator **directly in the current environment with no container at all** — for when the environment already IS the image, e.g. a rented pod booted *from* `environment.image`. This is what makes image hills work on hosts where nesting is impossible (an unprivileged pod with no user namespaces / no CAP_SYS_ADMIN): the pod is the toolchain, and the evaluator runs natively. In host mode the evaluator runs under the image's own Python (found on PATH with the launcher's ephemeral venv removed, or `HILLS_HOST_PYTHON` if set) so it sees the image's installed packages, not the interpreter a wrapper like `uv tool run` used to start hills. The tool never configures storage either — a node whose home is on NFS points podman's store at local disk itself (e.g. `CONTAINERS_STORAGE_CONF`). The image is the environment, so there is no uv sync for image hills; the image provides Python (the shim is standard-library only) and the hill's toolchain. Pin by digest, not a mutable tag: the image contents are not part of the hill's tree hash, only this reference is, so a moved tag would change behavior invisibly while a changed reference is a new hill version. A hill that declares an image fails on a machine with no runtime, with a clear message; hills without an `environment` are unaffected.
...
**Authoring half.** The agent interviews the user briefly in plain ML language — what the number stands in for, metric and direction, what data is held out, how much feedback the evaluator should expose — where every question exists so the hill measures the user's actual goal rather than something adjacent to it. It then scaffolds with `hills new`, extracts and freezes the evaluation logic from the user's code into the hill (so the hill's judgment cannot drift when the project changes), sorts content across the private boundary, writes the README-as-contract, a minimal `examples/` submission, and `tests/`. It then attacks its own draft. Findings sort into two piles. **Plumbing leaks** have a right answer and are closed without asking (metric computed by submitted code instead of the evaluator; hidden-test information leaking through error messages; timing gamed because the clock isn't evaluator-owned; stale artifacts; evaluator drift). A **gap between the metric and the goal** — a way of scoring well that skips the work, such as the closed form of a toy dataset or scaling up something the metric doesn't constrain — is measured, not unilaterally closed: closing it usually means changing the user's task, so it goes to the user with the size of the gap and a range of responses ordered by cost (state the goal in the README / report a diagnostic / constrain it in the evaluator / add an LLM or programmatic judge / change the task), with a recommendation for the smallest one the goal survives. Defensive authoring rule: do not open private data files beyond sniffing headers to confirm format, and say so. When the user's goals genuinely decompose into distinct objectives (e.g. quality at a fixed time budget *and* raw throughput), the agent proposes creating multiple hills, concretely — never as an open-ended menu.

**The decision brief and the commit.** Before anything is frozen, the agent presents in its message: (1) the user's goal in one sentence and how this hill measures it; (2) plumbing leaks it closed and how; (3) each gap it measured between the metric and the goal — presented as the user's design decision with concrete options and a recommendation. Then the **human** runs `hills commit` themselves. This is deliberate: the agent that wrote the evaluator must not be the one to freeze it.
